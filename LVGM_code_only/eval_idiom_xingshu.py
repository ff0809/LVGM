...
def get_args_parser():
    ...
    parser.add_argument('--fixed_draw_length', default= 96, type=int, help = 'Padding to fixed length of drawing instructions')
    parser.add_argument('--fixed_stroke_length', default= 24, type=int, help = 'Padding to fixed length of one word strokes')
    parser.add_argument('--batch_size', default = 1, type = int, help = 'Batch size per GPU')
    parser.add_argument('--output_dir', default='eval_outputtry_xingshu', help='Path where to save, empty for no saving')
    ...
    parser.add_argument('--prompt', default="临渊羡鱼", type = str, help='prompt characters for words generation')# 云
...
def main(args):
    ...
    for sp, data in enumerate(dataset):
        ...
        for tokens in words: # tokens里面是一个字的全部笔画token stack在一起
            if tokens.shape[0] % EMBEDNUM:
                tokens = tokens[0:tokens.shape[0]//EMBEDNUM*EMBEDNUM]
            strokes = encoder.get_zq_from_id(tokens, args.device, EMBEDNUM)
            strokes = decoder(strokes)
            strokes = strokes.reshape(tokens.shape[0]//EMBEDNUM, args.fixed_draw_length, 6)
            # print(strokes.shape)
