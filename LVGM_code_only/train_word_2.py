...
from vqvae.singleword_2 import MyDataset_stage_one as mydataset
...
def get_args_parser():
    ...
    parser.add_argument('--batch_size', default = 1, type = int, help = 'num of strokes for one tokenize') # 2个字
...
def build_dataset(args, tokenizer, encoder, max_text_len):
    ...
    def tokenize(sample):
        ...
        for dt in sample['text'][0]:
            ...
            cd = postdeal(list(map(int, cd))) # 单字 272个token
            code.extend(cd)
        # print(code)
