...
def vec2raster_1img(all_shapes, device, draw=False, canvas_size=64, color=torch.tensor([150, 150, 150, 255.0])/255, num=0):
    pydiffvg.set_use_gpu(torch.cuda.is_available())
    # torch.set_printoptions(sci_mode=False)
    # [N条笔画, 一张图里的n个指令]
    # seg1:[size, 64, 6]
    ...
    for shape in all:
        ...
        shapes.append(path)
        if colcnt >= colset:
            color = torch.tensor([0, 0, 0, 255])/255
        path_group = pydiffvg.ShapeGroup(
            shape_ids = torch.tensor([len(shapes)-1]).to(device),
            fill_color = color
        )
...
def vec2raster_1img_new(all_shapes, device, draw=False, canvas_size=64, color=torch.tensor([150, 150, 150, 255.0])/255, num=0):
    pydiffvg.set_use_gpu(torch.cuda.is_available())
    # torch.set_printoptions(sci_mode=False)
    # [N条笔画, 一张图里的n个指令]
    # seg1:[size, 64, 6]
