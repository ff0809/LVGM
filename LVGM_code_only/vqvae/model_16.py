...
class Encoder_stage_one(nn.Module):
    def __init__(self, input_dim=6, h_dim=1024, res_h_dim=256, embed_dim=32, num_embeds=30000, beta=.25): # 3w2
        super().__init__()
        ...
        self.embeds = nn.Embedding(num_embeds, embed_dim)#codebook
    ...
    def forward(self, x, device):
        ...
        ze = ze.permute(0, 2, 3, 1).contiguous() # 128, 4, 4, 16
        z_flattened = ze.view(-1, self.embed_dim) # 2048, 16
        d = torch.sum(z_flattened ** 2, dim=1, keepdim=True) + \
            torch.sum(self.embeds.weight ** 2, dim=1) - \
            2 * torch.matmul(z_flattened, self.embeds.weight.t()) # 2048, 8192距离

        min_encoding_indices = torch.argmin(d, dim=1).unsqueeze(1) # 2048, 1 压缩了128*4*4之后获得的每个原16对应距离最小的embeds的index
        min_encodings = torch.zeros(min_encoding_indices.shape[0], self.num_embeds).to(device) # 2048, 8192
        min_encodings.scatter_(1, min_encoding_indices, 1) # 2048, 8192

        zq = torch.matmul(min_encodings, self.embeds.weight).view(ze.shape) # 128, 4, 4, 16
        ...
        zq = zq.permute(0, 3, 1, 2).contiguous() # 128, 16, 4, 4
    ...
    def get_zq_from_id(self, min_encoding_indices, device):
        ...
        zq = zq_flattened.view((min_encoding_indices.shape[0]//16, 4, 4, self.embed_dim))#因为压缩128*4*4，所以对应是4*4=16，16个整数index对应一个stroke编码
        zq = zq.permute(0, 3, 1, 2).contiguous()
        return zq

class Decoder_stage_one(nn.Module):
    def __init__(self, in_dim=32, h_dim=1024, res_h_dim=256, out_dim=6):
        super().__init__()
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(in_dim, h_dim, 3, 1, 1),
            ResBlock(h_dim, h_dim, res_h_dim),
            ResBlock(h_dim, h_dim, res_h_dim),
            nn.ConvTranspose2d(h_dim, h_dim // 2, 3, 1, 1),
            nn.ReLU(),
            nn.ConvTranspose2d(h_dim // 2, out_dim, 4, 2, 1)
        ) #与encoder对应的解码结构，完全对称
        self.apply(weights_init)
