...
class Encoder_stage_one(nn.Module):
    ...
    def forward(self, x, device):
        ...
        ze = ze.permute(0, 2, 3, 1).contiguous() # 128, 2, 4, 16
    ...
    def get_zq_from_id(self, min_encoding_indices, device):
        ...
        zq = zq_flattened.view((min_encoding_indices.shape[0]//8, 2, 4, self.embed_dim))#因为压缩128*2*4，所以对应是2*4=8，8个整数index对应一个stroke编码
        zq = zq.permute(0, 3, 1, 2).contiguous()
        return zq
