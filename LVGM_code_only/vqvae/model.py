...
class Encoder_stage_one(nn.Module):
    ...
    def forward(self, x, device):
        ...
        z_flattened = ze.view(-1, self.embed_dim) # 512, 16
        d = torch.sum(z_flattened ** 2, dim=1, keepdim=True) + \
            torch.sum(self.embeds.weight ** 2, dim=1) - \
            2 * torch.matmul(z_flattened, self.embeds.weight.t()) # 512, 8192距离

        min_encoding_indices = torch.argmin(d, dim=1).unsqueeze(1) # 512, 1 压缩了128*2*2之后获得的每个原16对应距离最小的embeds的index
        min_encodings = torch.zeros(min_encoding_indices.shape[0], self.num_embeds).to(device) # 512, 8192
        min_encodings.scatter_(1, min_encoding_indices, 1) # 512, 8192
        ...
        zq = zq.permute(0, 3, 1, 2).contiguous() # 128, 8, 2, 2
