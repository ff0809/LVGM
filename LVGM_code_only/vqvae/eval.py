...
# from singleword import MyDataset_stage_one
from model_8 import Encoder_stage_one, Decoder_stage_one
...
def get_args_parser():
    ...
    parser.add_argument('--svg_path', default='/home/tongji209/latest/codeanddata/testtry/2', help = 'SVG saved dir')
...
def main(args):
    ...
    for sp, (strokes) in enumerate(data_loader_train):
        ...
        if 0:
            ...
            strokes = strokes.reshape(B*N, 6, 8, 8) 
            stroke_encode, stroke_vq, emb_loss, _ = encoder(strokes, device)
            strokes_pred = decoder(stroke_vq)
            
            strokes = strokes.reshape(B, N, 64, 6)
            strokes_pred = strokes_pred.reshape(B, N, 64, 6)
            # print(strokes.shape)
            ...
            strokes = strokes.reshape(B, 64, 6)
            strokes_pred = strokes_pred.reshape(B, 64, 6)
