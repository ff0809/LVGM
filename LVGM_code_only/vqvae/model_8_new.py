...
class Encoder_stage_one(nn.Module):
    def __init__(self, input_dim=6, h_dim=1024, res_h_dim=256, embed_dim=16, num_embeds=30000, beta=.25): # 3w2
        ...
        self.encoder = nn.Sequential(
            ...
            nn.ReLU(True),
            nn.Conv2d(h_dim // 2, h_dim, (4,3), (2,1), 1),
            nn.ReLU(True),
    ...
    def forward(self, x, device):
        ...
        ze = ze.permute(0, 2, 3, 1).contiguous() # 128, 2, 4, 16 [ , 3, 4, 16]
        ...
        zq = torch.matmul(min_encodings, self.embeds.weight).view(ze.shape) # 128, 2, 4, 8
        # print(zq.shape)
    ...
    #     return zq
    def get_zq_from_id(self, min_encoding_indices, device, embednum=8):
        
        zq_flattened = self.embeds.weight[min_encoding_indices]
        # print(zq_flattened.shape)
        zq = zq_flattened.view((min_encoding_indices.shape[0]//embednum, 2+(embednum==12), 4, self.embed_dim))#因为压缩128*2*4，所以对应是2*4=8，8个整数index对应一个stroke编码
        zq = zq.permute(0, 3, 1, 2).contiguous()
        return zq
...
class Decoder_stage_one(nn.Module):
    def __init__(self, in_dim=16, h_dim=1024, res_h_dim=256, out_dim=6):
        ...
        self.decoder = nn.Sequential(
            ...
            ResBlock(h_dim, h_dim, res_h_dim),
            nn.ReLU(True),
            nn.ConvTranspose2d(h_dim, h_dim // 2, (4,3), (2,1), 1),
            nn.ReLU(True),
