...
class MyDataset_stage_one(torch.utils.data.Dataset):
    def __init__(self, svg_path, idiom_path=None, ci_path=None, shi_path=None, max_stroke_sum=68, 
        fixed_stroke_length=24, fixed_draw_length=96, canvas_size=1024, prompt=None, is_normalization=True, is_pad=True, transform = None):
        super().__init__()
        word_list = {}
        word_stroke_len = {}
        ...
        for file in os.listdir(svg_path):
            ...
            for g in root.findall('{http://www.w3.org/2000/svg}g'):
                if g.attrib.get('transform'):
                    ...
                    for path in g.findall('{http://www.w3.org/2000/svg}path'):
                        ...
                        if len(pro_stroke) <= fixed_draw_length:
                            extension_count = self.fixed_draw_length - len(pro_stroke)
                            extension_elements = [self.empty_token] * extension_count
                            pro_stroke.extend(extension_elements)
                            if len(pro_stroke) != self.fixed_draw_length:
                                print('draw length error {}'.format(len(pro_stroke)))
            ...
            maxord_num = max(maxord_num, len(storke_list))
            ...
            word_list[file.split('.')[0].split('-')[0].split('_')[0]] = storke_list
            word_stroke_len[file.split('.')[0].split('-')[0].split('_')[0]] = cnt
        ...
        if ci_path is not None:
            ...
            # ci_data = random.sample(ci_data, 100000)
            for dt in ci_data:
                wd = dt['ci']
                if '(' in wd:
                    continue
        ...
        if shi_path is not None:
            ...
            if self.word_list == {}:
                self.word_list = []
            # shi_data = random.sample(shi_data, 10)
            # shi_data = random.sample(shi_data, 50000)
            for dt in shi_data:
                # wd = dt['content'].split('，|\n|。') 这个不行
                wd = re.split('，|。|\n', dt['content'])
                for st in wd:
