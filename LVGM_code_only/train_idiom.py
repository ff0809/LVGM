...
TOKENNUM = 30000 #码本编码总数
EMBEDNUM = 8 #每个笔画用8个整数表示
BOS_TOKEN = TOKENNUM + 1 #开始标记
EOS_TOKEN = TOKENNUM + 2 #结束标记
PAD_TOKEN = TOKENNUM + 3 #填充标记
SEP_TOKEN = TOKENNUM + 4 #分隔标记，区分不同字的笔画编码
...
ENDEMBED = [19444, 19444, 19444, 19444, 19229, 29852, 2443, 18889] #空笔画指向的编码
...
def get_args_parser():
    ...
    parser.add_argument('--fixed_stroke_length', default = 34, type=int, help = 'Padding to fixed length of one word strokes')# 一个字固定笔画数 楷书33 行书23
    ...
    parser.add_argument('--max_stroke_sum', default = 100, type = int, help = 'Max sum of strokes')
    return parser
...
def main(args):
    ...
    tokenizer = AutoTokenizer.from_pretrained("./deepseek-coder-1.3b-base") #修改库文件，禁用deepseek的tokenizer的pad token报错
