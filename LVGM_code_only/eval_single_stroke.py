...
from vqvae.diffvg_f_new import vec2raster_1img as vec2raster
import pydiffvg
...
def get_args_parser():
    ...
    parser.add_argument('--svg_path', default='/home/tongji209/latest/codeanddata/testtry/矢', help = 'SVG saved dir')
    ...
    parser.add_argument('--canvas_size', default= 2048, type=int, help= 'Canvas size of SVG')
    ...
    for sp, data in enumerate(dataset):
        ...
        try:
            ...
            if tokens_pred.shape[0]:
                nm = tokens_pred.shape[0]//EMBEDNUM
                mixout=vec2raster(strokes, args.device, draw=True, canvas_size=args.canvas_size, num=0)
                for i in range(len(mixout)):
                    img = mixout[i].detach().cpu().numpy()
                    pydiffvg.imwrite(img, args.output_dir+'/img'+str(sp)+'_in_'+str(args.canvas_size)+'.png')
                for cc in range(nm):
                    tkp = tokens_pred[cc*EMBEDNUM:(cc+1)*EMBEDNUM]
                    strokes_pred = encoder.get_zq_from_id(tkp, args.device)
                    strokes_pred = decoder(strokes_pred)
                    strokes_pred = strokes_pred.reshape(tkp.shape[0]//EMBEDNUM, 64, 6)
                    ...
                    mixout=vec2raster(strokes_pred, args.device, draw=True, canvas_size=args.canvas_size, num=0)

                    for i in range(len(mixout)):
                        img = mixout[i].detach().cpu().numpy()
                        pydiffvg.imwrite(img, args.output_dir+'/img'+str(sp)+'_'+str(cc)+'_'+str(args.canvas_size)+'.png')
            # print(strokes.shape)
            ...
            #     pydiffvg.imwrite((1-img), args.output_dir+'/img'+str(sp+i)+'_'+str(args.canvas_size)+'.png')
        except RuntimeError:
            continue
